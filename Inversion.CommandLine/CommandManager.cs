...
namespace Inversion.CommandLine
{
    ...
    public class CommandManager : ICommandManager
    {
        ...
        public ICommand GetCommand(string commandName)
        {
            ...
            if (!results.Any())
            {
                throw new CommandLineException(InversionResources.UnknownCommandError, commandName);
            }
            ...
            if (results.Skip(1).Any())
            {
                ...
                if (matchedCommand == null)
                {
                    ...
                    throw new CommandLineException(String.Format(CultureInfo.CurrentCulture, InversionResources.AmbiguousCommand, commandName,
                ...
                foreach (OptionAttribute attr in propInfo.GetCustomAttributes(typeof(OptionAttribute), true))
                {
                    if (!propInfo.CanWrite && !TypeHelper.IsMultiValuedProperty(propInfo))
                    {
                        ...
                        throw new InvalidOperationException(String.Format(CultureInfo.CurrentCulture,
                            InversionResources.OptionInvalidWithoutSetter, command.GetType().FullName + "." + propInfo.Name));
                    }
